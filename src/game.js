...
const MAP_CONFIG = {
  jungle:{bg:0x17372c,terrain:[0x244e3d,0x2f5e48,0x173026,0x3b5b31],gate:{x:1170,y:500},path:[{x:-40,y:260},{x:220,y:260},{x:290,y:140},{x:500,y:140},{x:590,y:360},{x:820,y:360},{x:910,y:190},{x:1130,y:190},{x:1200,y:500},{x:1320,y:500}],pads:[[190,150],[330,290],[450,80],[530,250],[670,470],[760,270],[930,120],[980,340],[1080,110],[1080,420]]},
  canyon:{bg:0x3d2f22,terrain:[0x5f4730,0x7d5e3c,0x3a2a1d,0xa07945],gate:{x:1165,y:210},path:[{x:-40,y:560},{x:150,y:500},{x:295,y:585},{x:420,y:410},{x:560,y:430},{x:705,y:230},{x:900,y:250},{x:1015,y:120},{x:1160,y:245},{x:1320,y:210}],pads:[[145,410],[285,455],[380,620],[505,315],[615,535],[710,335],[825,150],[920,360],[1045,85],[1120,335]]},
  marsh:{bg:0x18392f,terrain:[0x14352d,0x1f4c40,0x2f5e48,0x456436],gate:{x:1160,y:545},path:[{x:-40,y:115},{x:170,y:120},{x:250,y:315},{x:420,y:315},{x:520,y:520},{x:690,y:515},{x:785,y:335},{x:970,y:335},{x:1060,y:545},{x:1320,y:545}],pads:[[150,220],[270,80],[345,395],[470,205],[555,615],[675,420],[790,235],[920,445],[1065,310],[1135,620]]}
};
...
const UI = {
  ...
  renderMaps(){ if(!this.mapList)return; this.mapList.innerHTML=''; MAPS.forEach(map=>{ const unlocked=(PROGRESS.unlocked||['jungle']).includes(map.id); const span=document.createElement('span'); span.className='pill '+(STATE.mapId===map.id?'boss':''); span.textContent=unlocked?map.name:`${map.name} 🔒`; span.onclick=()=>{ if(!unlocked){this.log('Map locked. Reach later waves to unlock it.');return;} if(STATE.mapId===map.id)return; STATE.mapId=map.id; PROGRESS.currentMap=map.id; saveProgress(); window.location.reload(); }; this.mapList.appendChild(span); }); },
  ...
  renderPreview(){
    ...
    const eliteText = STATE.wave >= 5 ? ' Elite variants may appear.' : ''; if(this.previewHint)this.previewHint.textContent = STATE.wave>=STATE.maxWaves ? 'Survive the remaining dinosaurs.' : STATE.activeWaves > 0 ? `You can launch early. Active waves: ${STATE.activeWaves}.` : `Prepare defenses before starting.${eliteText}`;
  },
  render(){
