...
const UI = {
  ...
  render(){
    ...
    this.info.innerHTML=`<strong>${t.def.name}</strong> · ${t.def.path}<br>Level ${t.level}/5 · ${t.def.type}<br>Damage ${Math.round(t.damage)} Range ${Math.round(t.range)}<br>${t.nextPerk()}<br>Upgrade $${cost}`;
    this.upgrade.disabled=STATE.credits<cost||t.level>=5; this.sell.disabled=false;
  }
};
...
class Tower {
  ...
  upgradeCost(){ return Math.round(this.def.cost*(.7+this.level*.62)); }
